...
contract LPC is Ownable, Pausable, BlackListable, IERC20Metadata {
    ...
    event ParentsRewardsPaid(
        address child,
        address parent,
        uint256 reward,
        bool isDirect
    );
    event DividendsPaid(uint256 amount, uint256 holdersCount);

    constructor(address _rlink) {
    ...
    function balanceOf(address account) public view override returns (uint256) {
        uint256 capital = _balances[account];
        if (capital == 0) {
            return 0;
        }
        return
            _isValidRewardHolder(account)
                ? capital.add(calcPendingReward(account))
                : capital;
    }
    ...
    function _transfer(
        ...
        uint256 totalHolders_ = totalHolders;
        (bool vs, uint256 senderBalance) = _updateBalance(sender);
        (bool vr, uint256 recipientBalance) = _updateBalance(recipient);
        ...
        if (
            ...
            if (feeAmounts.feeAmount > 0) {
                ...
                (, uint256 feeToCapital) = _updateBalance(feeTo_);
                if (feeToCapital == 0) {
                    totalHolders_ += 1;
                }
                ...
                if (parent != address(0)) {
                    ...
                    if (feeAmounts.grandpaAmount > 0) {
                        ...
                        if (grandpa != address(0)) {
                            recipientAmount =
                                recipientAmount -
                                feeAmounts.grandpaAmount;
                            (bool v, uint256 gb) = _updateBalance(grandpa);
                            if (v && gb == 0) {
                                totalHolders_ += 1;
                            }
                            _balances[grandpa] = gb.add(
                                feeAmounts.grandpaAmount
                            );
                            emit Transfer(
                                sender,
            ...
            if (feeAmounts.holdersAmount > 0) {
                if (totalHolders_ > 0) {
                    recipientAmount =
                        recipientAmount -
                        feeAmounts.holdersAmount;
                    rewardPerHolderStored =
                        rewardPerHolderStored +
                        feeAmounts.holdersAmount /
                        totalHolders_;
    ...
    function _updateBalance(address account) internal returns (bool, uint256) {
        ...
        returns (FeeAmounts memory fees)
    {
        ...
        uint256 burnCap = _totalSupply - rates_.burnStopSupply;
        fees.burnAmount = amount.mul(rates_.burnRate).div(1e9);
        uint256 burnedAmount = balanceOf(_blackHole);
        if (fees.burnAmount.add(burnedAmount) > burnCap) {
            fees.burnAmount = burnCap > burnedAmount
                ? burnCap - burnedAmount
                : 0;
        }
    ...
    function _isValidRewardHolder(address _account)
        internal
        view
        returns (bool)
    {
    ...
    function setFeeRates(
        ...
        uint256 parentRate,
        uint256 grandpaRate,
        uint256 burnStopSupply
    ) public onlyOwner {
        require(
            burnRate.add(feeRate).add(holdersRate).add(parentRate).add(
                grandpaRate
            ) <= 1e9,
            "sum of rates can not greater than 1e9"
        );
        require(
            burnStopSupply <= _totalSupply,
            "burn stop supply can not greater than total supply"
        );
    ...
    function setFeeTo(address _feeTo) public onlyOwner {
        require(
            _feeTo != address(0) && !Address.isContract(_feeTo),
            "fee to can not be address 0 or contract address"
        );
        feeTo = _feeTo;

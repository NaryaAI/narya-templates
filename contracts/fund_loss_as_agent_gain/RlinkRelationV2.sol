...
// File: rlink/interfaces/IRlinkRelationV2.sol
...
interface IRlinkRelation {
     ...
     */
    function addRelation(address _child, address _parent)
        external
        returns (uint256);
     ...
     */
    function isParent(address child, address parent)
        external
        view
        returns (bool);
    ...
    event Distributed(
        ...
        uint256 toAmount,
        address parent,
        uint256 parantAmount,
        address grandpa,
        uint256 grandpaAmount
    );
}
// File: rlink/interfaces/ILegacyRlinkRelation.sol

pragma solidity ^0.8.0;

interface ILegacyRlinkRelation {
    function isParent(address child, address parent)
        external
        view
        returns (bool);
    ...
    function verifyChild() external view returns (bool);
}
// File: rlink/interfaces/IStakingTokenBurn.sol

pragma solidity ^0.8.0;

interface IStakingTokenBurn {
...
// File: rlink/rlinkRelationV2.sol

pragma solidity ^0.8.0;

contract RlinkRelationV2 is Pausable, BlackListable, IRlinkRelation {
    using Address for address;
    ...
    ILegacyRlinkRelation public immutable legacyRlinkRelation;
    ...
    event MaxStakeMonthChanged(
        address sender,
        uint256 oldLimit,
        uint256 newLimit
    );

    event GlobalTrialTimeChanged(
        address sender,
        uint256 oldTrialTime,
        uint256 newTrialTime
    );
    ...
    constructor(address _stakingToken, address _legacyRlinkRelation) {
        stakingToken = _stakingToken;
        legacyRlinkRelation = ILegacyRlinkRelation(_legacyRlinkRelation);
        createTime = block.timestamp;
        feeTo = msg.sender;
    }

    function bindReward() external view override returns (uint256) {
        return legacyRlinkRelation.bindReward();
    }

    function remainingRewards() external view override returns (uint256) {
        return legacyRlinkRelation.remainingRewards();
    }

    function mintedRewards() external view override returns (uint256) {
        return legacyRlinkRelation.mintedRewards();
    }
    ...
    function isParent(address child, address parent)
        ...
        returns (bool)
    {
        return legacyRlinkRelation.isParent(child, parent);
    }

    function parentOf(address account)
        external
        view
        override
        returns (address)
    {
        if (queryVerify) {
            require(
                _checkStake(msg.sender),
                "Rlink::distribute: insufficient stakes"
            );
        }

        return legacyRlinkRelation.parentOf(account);
    }

    function verifyChild() external view returns (bool) {
        return legacyRlinkRelation.verifyChild();
    }
    ...
    function addRelation(address _child, address _parent)
        ...
        returns (uint256)
    {
        IERC20 stakingToken_ = IERC20(stakingToken);
        uint256 oldRltBalance = stakingToken_.balanceOf(address(this));
        require(
            legacyRlinkRelation.addRelation(_child, _parent),
            "Rlink::addRelation: add relation failed"
        );
        uint256 newRltBalance = stakingToken_.balanceOf(address(this));

        if (newRltBalance > oldRltBalance) {
            stakingToken_.safeTransfer(
                msg.sender,
                newRltBalance - oldRltBalance
            );
        }

        emit AddedRelation(_child, _parent);
        return
            newRltBalance > oldRltBalance ? newRltBalance - oldRltBalance : 0;
    }

    function stake(address forAccount, uint256 amount)
        external
        override
        whenNotPaused
        notBlackListed
    ...
    {
        require(
            forAccount != address(0),
            "Rlink::stake: can not stake for address 0"
        );
        ...
        require(
            newAmount <= maxStakeAmount(),
            "Rlink::stake: total staked of forAccount exceeds max stake amount"
        );
        ...
        if (newStakedMonthes > oldStakedMonthes) {
            uint256 oldExpireAt = Math.max(
                _stakes[forAccount].stakedExpireAt,
                trialExpireAt(forAccount)
            );
            _stakes[forAccount].stakedMonthes = newStakedMonthes;
            _stakes[forAccount].stakedExpireAt = Math
                .max(oldExpireAt, block.timestamp)
                .add(newStakedMonthes.sub(oldStakedMonthes).mul(monthScale));
        }
    ...
    function expireAt(address account) public view override returns (uint256) {
        ...
        return
            Math.max(_stakes[account].stakedExpireAt, trialExpireAt(account));
    }

    function calcStakeMonthes(uint256 stakedAmount)
        public
        view
        returns (uint256)
    {
    ...
    function burnExpiredStake(address account)
        external
        override
        whenNotPaused
        notBlackListed
    {
        ...
        require(
            createTime.add(10 * 31 * 86400) < block.timestamp,
            "Rlink::burnExpiredStake: contract must created 10 monthes"
        );
        require(
            expireAt(account) < block.timestamp,
            "Rlink::burnExpiredStake: stake is not expired"
        );
        require(
            _stakeBalances[account] > 0,
            "Rlink::burnExpiredStake: account has no stakes"
        );

        uint256 stakeBalance = _stakeBalances[account];
        _stakeBalances[account] = 0;
        IStakingTokenBurn(stakingToken).burn(stakeBalance);
        ...
        returns (uint256 distributedAmount)
    {
        ...
        require(
            to != address(0),
            "Rlink::distribute:  to address can not be address 0"
        );
        require(
            incentiveAmount.add(parentAmount).add(grandpaAmount) <= amount,
            "Rlink::distribute: sum of the rates must less than 1e18"
        );
        ...
        address parent_ = legacyRlinkRelation.parentOf(to);
        ...
        if (parent_ != address(0) && parentAmount.add(grandpaAmount) > 0) {
            ...
            if (grandpaAmount > 0) {
                grandpa_ = legacyRlinkRelation.parentOf(parent_);
    ...
    function trialExpireAt(address account)
        public
        view
        override
        returns (uint256)
    {
        ...
        uint256 checkStartAt = firstCallTs == 0
            ? block.timestamp
            : Math.max(stakeEnableTime, firstCallTs);
    ...
    function setDistributeFee(uint256 newFee) external onlyGovernance {
        require(
            newFee <= distributeFeeCap,
            "newFee exceeds distribute fee cap"
        );
    ...
    function setSpecialTrial(address account, uint256 trialSeconds)
        external
        onlyGovernance
    {
    ...
    function setMaxStakeMonth(uint256 newMaxStakeMonth)
        external
        onlyGovernance
    {
